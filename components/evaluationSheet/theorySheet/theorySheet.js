...
Component({
  ...
  methods: {
    ...
    // resolveContentData(obj, newObj) {
    //   for(let i in obj) {
    //     // newObj[i] = obj[i]
    //     if(typeof(obj[i]) == 'object' && i != 'evaluationList') {
    //       this.resolveContentData(obj[i], newObj)
    //     }else {
    //       newObj[i] = obj[i]
    //     }
    //   }
    ...
    //         for(let j of arr) {
    //           if(!this.data.contentData[i][j] || this.data.contentData == '') {
    //             err = j
    //             break
    //           }
    //         }
    ...
    // 点击提交按钮
    submitForm() {
      dealAndSubmitForm(this, 'theoryEvaluation')
    ...
    //       resolveObj(this.data.contentData, this.data.formData)
    //       // 修改一些属性的名称
    ...
    //       // this.dealTime()
    //       dealTimeFormat(this.data.formData)
